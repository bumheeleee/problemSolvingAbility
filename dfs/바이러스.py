...
for _ in range(paris):
    node1, node2 = tuple(map(int, input().split()))
    #양방향 매핑
    rel_arr[node1][node2] = 1
    rel_arr[node2][node1] = 1

visited = [False] * (n+1)

def dfs(st_node):
    # 해당 노드와 연관된 노드를 dfs탐색, 1 ~ n번 노드까지 탐색
    for rel in range(1, n+1):
        rel_node = rel_arr[st_node][rel]
        if rel_node and visited[rel] == False:
            visited[rel] = True
            dfs(rel)
